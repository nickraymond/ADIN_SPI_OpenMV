...
| D34 | 2026-08-16 | **S18 bite B — the bench control socket and still-save** (Nick approved plan + 4 decision points). The telemetry role carries a loopback JSON control socket at `/run/bm/bench.sock` — **AF_UNIX SOCK_DGRAM**, non-blocking, drained from `loop()`, copying the shipped `gateway_ipc` listener's shape; one datagram is one complete message, so there is no framing code, no partial-read buffer and no connection table, and no port exists on any interface. Verbs map **1:1 onto the FIFO CLI's own handlers** (same `send_camera_req`/`send_light_req`, same `parse_res`/`parse_pf` pass-through), so the socket and the operator CLI cannot drift; out-of-range geometry stays the SERVICE's refusal to make (D31). Every request gets exactly one reply, including malformed ones — bm-cmd.sh's rule applied to the socket. Still-save arms inside `send_camera_req`, so a capture typed by hand is saved exactly like one from the web tool; `.tmp`+rename with the JPEG written before the sidecar makes **the sidecar the commit record** (bite C's gallery enumerates sidecars). Nothing is ever deleted; below a 200 MB floor the save is refused and counted. The whole parse/render surface lives in `bench_ctl.h` with no OS calls so it is testable on a laptop (98 checks). | A bench instrument's job is to produce trustworthy evidence, so every design choice favours "refuse and say so" over "proceed and hope": a truncated status is an error rather than half an object, a missing reply is marked `seen: false` rather than faked, and a still that arrives after the capture window times out loudly instead of silently not existing. DGRAM over STREAM because the message boundary is free and connection state is the thing that leaks. AF_UNIX over 127.0.0.1 because "loopback-only" should be structural, not a config line someone edits later. |
...
| D38 | 2026-08-18 | **S18 HD-stability bite — the bench runs PATCHED OpenMV firmware (sticky high-water framebuffer), and the HD-ref guard is lifted on measurement** (Nick: plan A root fix, shape A2, guard-until-measured, upstream HELD as item 11). Five probes (G–G5) reduced the matrix's two HD findings to one root cause: **sensor mode transitions degrade the board only while the HE core is resident** — no-HE control 52/52 lifetime vs HE-resident deaths at transition #10 (board off bus) and #22 (289 µs refusal to allocate 64 KB from the 2512K pool) with zero traffic; publish, the gate's barrier, HD, mono and the grow each individually exonerated; traffic only accelerates onset (on-chain N≈2). Source-corroborated at the installed rev (== master in the relevant files): `framebuffer_resize` frees and re-mallocs the ENTIRE framebuffer block on every transition (openmv lib/imlib/framebuffer.c:158, `UMA_MAYBE` → the polite MemoryError; corrupted free → the hard death; `raw_base=NULL` → B2's 13 µs instant-refusal wedge). Fix = `firmware/openmv_patches/0001-framebuffer-sticky-highwater.patch` (+9/−1): reuse the block whenever it fits, so the bridge's pre-HE HD ceiling claim becomes a true reservation and the allocator is never touched again after boot. Built at pinned `7d4dbf7`+patch via the D23/D24 loop (label `v5.0.0-52.g7d4dbf7ab2.dirty`), S7-ladder flashed byte-verified; **rollback = the stock set in `~/fw/development/`, one command**. Acceptance: the G3 soak 40/40 HE-loaded (stock fail-at-#22); on-chain HD ×7 ledger-exact; matrix HD stills byte-exact; **cert rung PASS 20.02 s → `REINIT_MIN_QUIET_MS = 20000` HD-certified**. Bridge `df82aa70…`: refusals hoisted above `_ensure_sensor` (the shipped guard ran the deadly re-init BEFORE refusing) and the HD-ref guard removed after probe G6 measured both ref-HD images loading + encoding with the HE resident. PublishGate itself is UNCHANGED. | Patching vendored firmware is a real architectural step — the bench's camera behaviour now depends on a repo-carried patch, so the patch file, the MANIFEST discipline and the one-command rollback are part of the design, not ops trivia. A2 beat the linker-static alternative because it is ~10 lines, needs no memory-map surgery, upstreams as a general churn fix, and achieves the same never-reallocate property given the existing eager ceiling claim. The guard lift follows the standing rule that measurement beats caution-by-default: G6 turned "suspected 2 MB load hazard" into "measured safe on fresh boots, measured failing on fragmented heaps" — so the guard became a preload design question (filed) instead of a blanket refusal. B2's gate stays because quiet windows measurably delayed onset on-chain and the cert rung now certifies the constant at HD. |

| D39 | 2026-08-18 | **Ladder resequenced (Nick): S22 (new — HE flood fix, then encoder-headroom exploration) runs before S21 (CV, promoted) which runs before S20 (light, delayed).** Sprint numbers are NOT renumbered — order changes, labels stay (the D30/D32 precedent). The encoder exploration is a measure-first bite: candidates (JPEG parameter space, capture/encode overlap, C-side pipeline tax, possible SoC JPEG/2D accel — datasheet-verify, never assume) each get a measured or sourced fps delta and an LoC/risk estimate before any code is approved. | Light auto-on is "not really a product offering yet" (Nick) while CV counting IS the product; both S22 bites precede CV because detector development wants a camera that cannot be wedged (finding 1 fixed, guardrails raised to the new boundary) and honest headroom numbers before anyone designs around today's encode-bound ceilings (28.07 / 7.40 / ~1.5–2.5 fps, derate 0.56–0.58, transport not the binder). Buying numbers before code is the S0 discipline applied to the encoder. |
...
| D46 | 2026-08-26 | **HIL screen→camera mapping is H + radial k1 (`CamMap`), solved from a 9-marker grid by least-squares DLT under a 1-D k1 search — and every consumer (harness scoring, hil_rescore, hil_report, hil_heatmap) goes through it.** Persisted as `calib_<board>.json` per run; the bare `H_<board>.npy` is still written and legacy runs load as k1=0, byte-identical to their original scores (pinned by test). The 4-corner DLT it replaces was exact at the markers and blind mid-field — measured 25.7 px of mid-field error on the N6 at HD (AE3 4.4 px), fatal at 30–40 px targets under IOU 0.30. Solver is numpy-only and deterministic (coarse k1 grid + parabolic refine; Newton on the radial cubic for the inverse — a fixed-point inverse measurably missed 1e-6 at N6-class k1); per-camera print = k1 (the lens meter) + fit RMS + leave-one-out RMS + the old DLT's mid-field error. Renderers needed zero change: page and hil-lcd both draw the marker list the playback server serves. | **The measured payoff rewrites a standing conviction: the N6's HD-tiny recall/precision went 0.43/0.30 → 0.781/0.593 — full parity with the AE3 flat control (0.783/0.574) — and the E8 IOU-0.20 discriminator jump collapsed +0.151→+0.009, identical to the AE3. The N6's "lens deficit" was ENTIRELY measurement artifact; the lens is guilty of distortion, not detection loss.** The E8-derived prediction (0.59/0.41) undershot because it was computed through the distorted mapping — loosening IOU cannot repair a systematic mid-field offset, only a correct transform can. On-board `img.lens_corr()` (which would also feed the model undistorted pixels) is priced at ~150 ms/frame at HD on the N6 (probe, measure-only) and NOT enabled: it changes the model's input mid-experiment, and parity arrived without it. |
| D45 | 2026-08-20 | **S25 workbench is a NEW module (`pi/workbench/`) on bench_web's patterns, not an extension of `bench_web.py` — and its runner is built around what it must never do.** Menu + passive preflight + runner on :8088, `workbench.service` ENABLED at boot (unlike every bm-* unit). Recipes are strict-schema TOML — unknown keys are ERRORS, a broken file renders as a red card naming the error, and the registry is re-read per request. Preflight opens NO serial port: by-id presence, /proc-fd port-holder scan (same-user visibility only — the lock, not the panel, is the correctness mechanism), disk, and only recipe-DECLARED systemd units (no standing list; three of S18's six belong to the parked two-Pi bench). The runner: one demo at a time (server-side 409), start refuses absent boards and names foreign port holders without killing them, LIVE is health-gated on the recipe's URL answering 200, stop is SIGINT→grace→SIGTERM→grace→STUCK-with-manual-command, `signal.SIGKILL` is pinned out of the source by a test, a pidfile lets a restarted workbench re-adopt a running demo, and a **35 s settle window** after stop/failure blocks restarts onto the same boards (page shows a countdown). Exposure per Nick: 0.0.0.0, loud banner, no auth, trusted LAN. | New-module reasons are measured, not aesthetic: `bench_web.py` deliberately "owns no hardware" and is itself a future menu entry, and the S8 viewer the first recipe launches already claims :8090 — the launcher must sit above both. The never-do list is the design because this is an unauthenticated control surface that can (from bite 3) reflash boards: the schema cannot express a DFU target (never alt 0), SIGKILL measurably took the N6 off the USB bus, killing a foreign holder could shoot an mpremote flash mid-write, and the settle window exists because Nick's quick stop→start wedged the AE3 into S23 bite R's power-cycle-only raw-repl refusal — two properly-serialised recovery attempts failed and only a physical replug cleared it. Boring, debuggable, refuses what it cannot undo. |

| D47 | 2026-09-07 | **A field rig's USB camera boards must have `usb-storage` unbound, and the rule matches interface CLASS (08/06/50 under VID 37c5), not product IDs.** Both OpenMV boards expose `/flash` as an MSC disk that this project never uses (all transfer is serial, via mpremote). A udev/SCSI probe landing while the board is busy fails a read, the kernel resets the USB **device**, the reset re-triggers the probe, and the pair livelocks — measured at **~46 resets/min** — with cdc_acm re-binding on every cycle. Shipped as `pi/field/usb_msc_off.sh` + `field-usb-msc-off.service`, enabled at boot, idempotent, and it also drops in the S7 DFU rule so flashing needs no root. | This one root cause had been diagnosed as **four separate bugs** across two rigs: a device missing from `lsusb`, a wedged serial port, "could not enter raw repl", and a firmware crash. It is recorded as a decision rather than a bench note because it changes what "the board is flaky" means on this project: before blaming hardware, rails, hubs or cables — the previous, and **wrong**, suspect list — check that MSC is unbound. The class-based match is deliberate: a product-ID match would silently lapse the day a firmware build reorders its interfaces, and the failure it protects against is invisible until a board disappears mid-run. |
| D48 | 2026-09-07 | **Two independent timeouts of the same magnitude on one path is a bug, not defence in depth — the field viewer therefore SERVES before it DISCOVERS.** The AE3's raw-REPL refusal is handled by waiting 60 s of guaranteed port silence (contact restarts the board's quiet-exit timer, so polling actively prevents recovery). The workbench separately health-gates a demo as LIVE on its page answering within 60 s. Composed, one grumpy board made discovery block the HTTP server past the gate, and the runner SIGINT'd the viewer **mid-wait** — killing all three cameras and surfacing as a `KeyboardInterrupt` inside `sleep()`. Fixed by binding the socket and rendering the page first, then discovering and attaching on a background thread; panels show "discovering board…" and fill in as boards answer. | Each mechanism was correct alone and both were written by the same author within a day of each other, which is exactly why it went unnoticed: the bug lives in the composition, not in either component, and no unit test that exercises one can see it. The general rule this puts on the project: any code that waits on hardware must be off the path that answers a liveness check, and a health gate must outlast the longest recovery wait beneath it — otherwise the supervisor kills the thing it is supervising precisely when recovery was working. The same shape had already bitten the composite card once (capture-then-serve exceeded the gate), so this is the second instance of one pattern. |
| D49 | 2026-09-07 | **The N6's hardware H.264 is NOT a custom-firmware question — it is an upstream OpenMV feature milestoned v5.1.0, and the block is already running on this bench as the N6's JPEG encoder.** Settled at the desk (S31, zero hardware): the STM32N657 carries a Hantro/VeriSilicon **VC8000NanoE** (ST CMSIS `stm32n657xx.h` — `VENC_BASE` = APB5+0x5000, `VENC_IRQn` 62, 128 KB dedicated VENC RAM at 0x24400000; ST LL driver `stm32n6xx_ll_venc.{h,c}`); OpenMV vendors the whole VC8000NanoE v9.22.3.7 package BSD-3-Clause (`drivers/vc8000/` @ `55d6fb90`) plus a complete EWL including `EWLMallocRefFrm` (`ports/stm32/stm_vc8000.c` @ `8af1f3d0`); **both halves compile into every shipping N6 image** (`drivers/drivers.mk:269`, `boards/OPENMV_N6/board_config.mk:33`); and `jpeg_compress()` already dispatches to it first (`ports/stm32/stm_jpeg.c:287-294`). The MicroPython binding — `codec.H264Encoder`, an MP4 muxer and an asyncio RTSP server — is open PR **openmv/openmv#3247** by OpenMV's own maintainer, `mergeable_state: clean`, all CI green, +58 KB of N6 text and −88 B on every other board. **Decision: do not fork; take it on release.** A branch build exists (`firmware/openmv_build/build_n6.sh --pr 3247`) so the win can be measured before v5.1.0 ships, and it has NOT been flashed. | The standing policy — test stock hardware, bail if a feature needs custom firmware — was written to avoid owning a fork: rebase burden, merge risk, no CI, no end date. **None of those costs are present here**, because the code is upstream's and the wait is one release. Recording it as a decision rather than a finding is deliberate: the next agent to hit "the N6 has an encoder Python can't reach" must not re-open the fork question, and must not re-derive that the 68.6 fps VGA MJPEG baseline and the AE3's 13.7 fps are the same measurement of *different silicon* (the AE3 declares neither `HAS_VENC` nor `HAS_JPEG`). The decision that stays open is Nick's and is about duty cycle, not engineering: at one 5 s clip per hour the entire MJPEG video budget is 4.68 kbps, so H.264 can return at most that; the tier where it changes feasibility rather than cost is 720p, where MJPEG's ~10.1 Mbps does not fit the 8 Mbps T1L budget and H.264's ~2.5 Mbps does. |
| D50 | 2026-09-07 night | **D49 is SUPERSEDED on two points by measuring it on hardware. (a) There is no v5.1.0** — no tag, no release, no due date; it is a milestone label and PR #3247 is still a **draft**, so "take it on release" describes a release that does not exist, and H.264 today means non-release firmware. **(b) The predicted ~4x does not survive max quality: at HD it is 1.5x** at matched quality (426,720 → 279,007 B/frame); the 3.2x / 6.1x / 11.9x wins at 32/16/8 Mbps are available only by accepting a quality ceiling. Root cause measured: re-encoding one *identical* frame costs a **43-byte** P-frame, a live *static* scene costs **257,704**, and locking AE/AWB changes that by 0.02% — so the entire P-frame cost is **sensor noise**, which has no temporal redundancy to exploit. P-frame as a fraction of an I-frame runs 20% at q50 → 80% at q90 → **99% at q100**. The firmware WAS built, flashed and rolled back (D49 said not flashed); the N6 is back on stock v5.0.1. **And the decisive finding is not about the codec at all: USB is not a bottleneck — 19.5 MB/s measured N6→Pi — while the stream delivers 3.6 fps against an encoder that does 30.5.** | D49 was reasoned from source and was right about the mechanism and the licensing; it was wrong wherever it substituted a plausible reading for a measurement — a release that was only a milestone label, and a compression ratio quoted without a quality target. Recorded as a NEW decision rather than an edit because D49's reasoning is still the correct reasoning; the lesson is what it missed. **A ratio with no quality attached is not a number, and a milestone is not a release.** The practical consequence is S32: the work is a faster frame-pump in software, not a codec and not a different board. Also closed by measurement — the N6's VC8000 has **no denoise filter** (`hwCfg.dnfSupport == 0`), so the one cheap lever against the noise cost does not exist. |
| D51 | 2026-09-08 | **A camera's DELIVERED frame rate, not its encoder rate, is what a recorder can promise -- and on these boards the gap is large.** Measured on nereus000's N6 (stock v5.0.1): HD q85 ENCODES at 34.9 fps and DELIVERS 23.8 end to end; HD q90 encodes at 29.3 and delivers 16.2. The cause is structural, not tuning: the board writes each JPEG over USB from inside the same single-threaded MicroPython loop that encodes it, so a 195 KB frame costs ~11 ms of link on top of 27 ms of encode, and a 423 KB frame costs ~28 ms on top of 33. Consequently `pi/field/camera_ceilings.json` carries BOTH numbers and the recorder card guards on `delivered` wherever a real recording has produced one, falling back to `cells` (encoder) with the caveat stated in the verdict text. Delivered rates are harvested from session manifests by `make_ceilings.py --recordings`, so the guard improves as the rig is used. | Guarding on the encoder number is not conservative, it is wrong in the dangerous direction: it would have told Nick that HD q85 at 30 fps was fine (34.9 > 30) and then handed back 23.8 -- a 21% shortfall discovered only after a dive. This repo has paid three times for the 'plausible artifact' class (S24's frozen streams), and a frame rate a UI promises but the hardware never reaches is the same bug wearing a UI. The encoder ceiling is still recorded because it isolates WHERE a cell is limited -- when delivered is far below encoder, the link is binding and a lower quality rung buys real fps; when they are close, the encoder is binding and only a smaller frame helps. |
| D52 | 2026-09-08 | **A firmware flash changes an OpenMV board's `/dev/serial/by-id/` path, so no config may pin one.** Flashing nereus000's N6 from v4.8.1 to stock v5.0.1 moved it from `usb-MicroPython_Pyboard_Virtual_Comm_Port_in_HS_Mode_0065345D3643-if01` to `...in_FS_Mode_10003500025043364d343000-if00`: the newer firmware reports the full 96-bit STM32 chip UID instead of a shortened form, and the interface number moved too. Boards are therefore resolved by ROLE (`pi/field/discover.py`, asking `omv.board_type()`), and `recorder.py` keeps a role->port HINT that is always re-verified by asking the board, so a stale hint is detected rather than trusted. | SPEC has said 'identify a board by asking it' since S8's mis-run benchmarked the wrong chip, and S29 proved by-id cannot even identify a RIG once boards move between hosts. This adds a third, sharper reason: by-id is not stable across a FIRMWARE UPDATE of the same physical board. The hint exists only so that recording from one camera does not open the other's port -- a raw-REPL attach on a board you are not using is not free, and repeated attaches are themselves how the AE3 wedges. |
...
byte-untouched) → browser at `http://nereus001:8080/stream` (that
hostname is the tailnet MagicDNS name — same URL from any tailnet
subnet; server binds 0.0.0.0, verified from off-subnet. Was
`nereus001-1` when this ran; tailnet names cleaned up 2026-08-14).
