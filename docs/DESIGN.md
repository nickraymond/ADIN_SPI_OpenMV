...
| D45 | 2026-08-20 | **The S25 workbench UI is served on the wifi with NO authentication (Nick).** The home page that lists and launches bench test setups binds `0.0.0.0` and is reachable by anything on the local wireless network. Recorded as a decision rather than an oversight, because this surface differs in kind from the stream viewer that preceded it: it can reflash boards, restart hardware and reconfigure `/flash`, so on that network it is an unauthenticated *control* surface. Two mitigations ride with the decision instead of auth: the loud bind banner the stream viewer already prints, and a hard rule that the runner never performs an action it cannot undo — DFU alt 0 (BOOTLOADER) is never written (S8 B1 established that a bad ROMFS write stays recoverable precisely because of this), and drift is repaired by bite 3's reconciliation rather than by a button that can leave a board unbootable. | The bench sits on a trusted home network and the cost of auth — a login on every phone, every session, for a tool whose whole point is "open it and press one button" — buys nothing against the actual threat model there. The honest framing is that the trust boundary IS the wifi, so the sprint's safety comes from bounding what the runner can destroy rather than from gating who can click it. That bound is worth having regardless of auth: a menu that can launch anything is exactly how the port-ownership wedges of 2026-08-20 would be repeated at one click, which is why the board lock and the never-write-BOOTLOADER rule are requirements and not preferences. Revisit if the bench ever leaves a trusted network. |
