...
def test_cell_center_lands_where_decode_np_puts_it():
    """Identity-ish homography: camera px == still px/1000. A cell at
    grid (y=1, x=2) of an hh=32 head (stride 8) in a tile at (192, 144)
    has center (192+(0.5+2)*8, 144+(0.5+1)*8) = (212, 156)."""
    Hinv = np.diag([1 / 1000.0, 1 / 1000.0, 1.0])   # cam px -> fractions
    grid = np.zeros((GRID_H, GRID_W), np.float32)
    accumulate(_rec([[192, 144]],
                    [[[32, 1, 2, 0.5, 0.5, 0.1, 0.1, 0.9, 0.8]]]),
               Hinv, grid, conf_floor=0.05)
...
def test_conf_floor_drops_noise_cells():
    Hinv = np.diag([1 / 1000.0, 1 / 1000.0, 1.0])
    grid = np.zeros((GRID_H, GRID_W), np.float32)
    accumulate(_rec([[0, 0]],
                    [[[32, 0, 0, 0.5, 0.5, 0.1, 0.1, 0.11, 0.2]]]),
               Hinv, grid, conf_floor=0.05)          # 0.022 < floor
    assert grid.sum() == 0.0


def test_out_of_frame_cells_are_dropped_not_wrapped():
    Hinv = np.diag([1.0, 1.0, 1.0])                  # frac >> 1
    grid = np.zeros((GRID_H, GRID_W), np.float32)
